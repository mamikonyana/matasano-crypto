...
def test():
    ...
    def norm_ham_dist(keysize):
        return (hamming_distance(raw_bytes[:2 * keysize],
                                 raw_bytes[2 * keysize:4 * keysize]) / keysize)

    keysize = min(range(2, 80), key=norm_ham_dist)
    message = decrypt(raw_bytes, keysize)
    first_line = '''I'm back and I'm ringin' the bell '''
    last_line = '''Play that funky music \n'''
    assert(message[:34] == first_line)
    assert(message[-23:] == last_line)
